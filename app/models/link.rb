...
class Link < ActiveRecord::Base
  ...
  scope :full_links, lambda { |current_user|
    select("links.*").join_voted_field(current_user).joins(:user)
  }

  scope :all_links, lambda { |current_user|
    full_links(current_user).order("created_at DESC")
  }

  scope :popular_links, lambda { |current_user|
    full_links(current_user).order("votes_count DESC").includes(:user).limit(20)
  }

  scope :new_links, lambda { |current_user|
    all_links(current_user).includes(:user).limit(20)
  }

  scope :by_id, lambda { |id, current_user|
    all_links(current_user).where(:id => id)
  }
